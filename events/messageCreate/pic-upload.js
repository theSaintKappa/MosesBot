...
module.exports = async (message, instance) => {
    ...
    const attachmentFileTypes = [...message.attachments.values()].map((attachment) => attachment.contentType);
    if (!['image/jpeg', 'image/png', 'image/gif', 'image/webp'].some((fileType) => attachmentFileTypes.includes(fileType))) return message.reply({ embeds: [embed.setTitle('Only allowed file formats are: PNG, JPG, GIF & WEBP').setColor('#ff0000')] });
    ...
    message.reply({ embeds: [embed.setTitle(`🎉 Succesfully uploaded ${message.attachments.size} pic${message.attachments.size > 1 ? 's' : ''}!`).setColor('#aaff29')] });
