import { CommandInteractionOptionResolver, EmbedBuilder, GuildMember, InteractionReplyOptions, PermissionsBitField, SlashCommandBuilder, User } from "discord.js";
import { CommandObject } from "../commands";
import { SchemaWithMetadata } from "../db/types";
import Leaderboard, { ILeaderboard } from "../models/moses/leaderboard.schema";
import Quote, { IQuote } from "../models/moses/quote.schema";
...
export default {
    builder: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addNumberOption((option) => option.setName("id").setDescription("An id of the quote you would like to edit. To check quote id's, run: /moses list <page>.").setRequired(true))
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addNumberOption((option) => option.setName("id").setDescription("An id of the quote you would like to delete. To check quote id's, run: /moses list <page>.").setRequired(true))
        )
    ...
    run: async (interaction) => {
        ...
        const footerText = `Moses quotes | request took: ${Date.now() - interaction.createdTimestamp}ms`;
        infoEmbed.setFooter({ text: footerText });
...
const infoEmbed = new EmbedBuilder().setColor("#00c8ff");
const successEmbed = new EmbedBuilder().setColor("#00ff3c");
...
async function list(page: number): Promise<InteractionReplyOptions> {
    ...
    const quotesWithMetadata = await Quote.aggregate<SchemaWithMetadata<IQuote[]>>([
        {
            $facet: {
    ...
    if (totalQuotes === 0) return { embeds: [errorEmbed.setDescription("❌ There are no quotes stored in the database.")], ephemeral: true };
    if (page > validPages)
        return {
...
async function add(quote: string, user: User): Promise<InteractionReplyOptions> {
    const leaderboardEntry = await Leaderboard.findOne<ILeaderboard[]>({ userId: user.id });

    if (!leaderboardEntry) {
        await Leaderboard.create({ userId: user.id, count: 1 });
    }

    if (!quote.endsWith(".") && !quote.endsWith("?") && !quote.endsWith("!")) quote += ".";
    await Quote.create({ id: 135, content: quote, submitterId: user.id });
...
async function edit(id: number, newQuote: string, memberPermissions: PermissionsBitField, user: User): Promise<InteractionReplyOptions> {
    ...
    if (memberPermissions.has(PermissionsBitField.Flags.Administrator)) {
        await Quote.updateOne({ id }, { content: newQuote });
...
async function leaderboard(): Promise<InteractionReplyOptions> {
    const leaderboard = await Leaderboard.find<ILeaderboard>();
    ...
    return { embeds: [infoEmbed.setTitle(`> 💛 Moses leaderboard:`).setDescription(leaderboardString)] };
}
