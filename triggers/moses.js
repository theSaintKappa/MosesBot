exports = async function (changeEvent) {
    ...
    const leaderboard = db.collection("moses.leaderboard");
    ...
    try {
        if (changeEvent.operationType === "insert") {
            ...
            const { id, submitterId } = await quotes.findOneAndUpdate({ _id: docId }, { $set: { id: sequence } }, { returnNewDocument: true });
            console.log(`Appended ${namespace.coll} document with id ${id}`);

            const { userId } = await leaderboard.findOneAndUpdate({ userId: submitterId }, { $inc: { count: parseInt(1, 10) }, $set: { updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } }, { returnNewDocument: true, upsert: true });
            ...
            const { userId } = await leaderboard.findOneAndUpdate({ userId: docBeforeChange.submitterId }, { $inc: { count: parseInt(-1, 10) }, $set: { updatedAt: new Date() } }, { returnNewDocument: true });
