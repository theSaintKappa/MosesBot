exports = async function (changeEvent) {
    ...
    try {
        if (changeEvent.operationType === "insert") {
            ...
            const { userId } = await leaderboard.findOneAndUpdate({ userId: submitterId }, { $inc: { count: parseInt(1, 10) } }, { returnNewDocument: true, upsert: true });
            ...
            const { userId } = await leaderboard.findOneAndUpdate({ userId: docBeforeChange.submitterId }, { $inc: { count: parseInt(-1, 10) } }, { returnNewDocument: true });
