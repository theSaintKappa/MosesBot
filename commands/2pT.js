const quotesSchema = require('../schemas/class-quotes-schema');
const counterSchema = require('../schemas/class-quote-counter-schema');
const { EmbedBuilder } = require('discord.js');
const { PermissionsBitField } = require('discord.js');

module.exports = {
    category: '2pTquotes',
    description: '2pTquotes',
    slash: true,
    testOnly: true,
    ...
    callback: async ({ interaction, user, member, client }) => {
        ...
        const edit = async (quoteId, newQuote) => {
            ...
            if (member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                await quotesSchema.updateOne({ quoteId }, { quote: newQuote })
                    .then(() => {
        ...
        const drop = async (quoteId) => {
            ...
            if (!member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                ...
                await quotesSchema.deleteOne({ quoteId })
                    .then(async () => {
